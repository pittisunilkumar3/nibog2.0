...
// IMPORTANT: Uses local date components to avoid timezone conversion issues
// that can cause dates to shift by one day when converting to UTC
export function formatDateForAPI(date: Date | string): string {
  if (date instanceof Date) {
    // Use local date components instead of toISOString() to prevent timezone issues
    // toISOString() converts to UTC, which can shift the date back by one day
    // when the local time is earlier than the UTC offset (e.g., IST is UTC+5:30)
