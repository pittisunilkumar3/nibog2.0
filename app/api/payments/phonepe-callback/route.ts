...
async function createBookingAndPayment(
  ...
  try {
    ...
    // No notifications sent - only booking creation
    return { success: true, bookingId };
  ...
  try {
    ...
    if (paymentState === "COMPLETED") {
      ...
      if (bookingResult.success && bookingResult.bookingId) {
        ...
        // Notifications are already sent by createBookingAndPayment or createBookingAndPaymentDirect functions
        ...
        return NextResponse.json({
          ...
          emailSent: true, // Indicate that email was sent by the booking creation functions
          notificationsSent: true // Indicate that notifications were sent
        });
      } else {
