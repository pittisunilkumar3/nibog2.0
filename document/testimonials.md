...
> Note: The current implementation accepts `image_url` directly. Use multipart only if your server supports file uploads — I can add upload handler code if you want automatic file storage and `image_url` generation.
...
Create (multipart — only if upload handler present):
```
